...
struct BlobConfig {
    ...
    blob_consistency: BlobConsistencyConfig<MAX_AGG_SNARKS>,
}
...
impl Circuit<Fr> for BlobCircuit {
    ...
    fn configure(meta: &mut ConstraintSystem<Fr>) -> Self::Config {
        ...
        let blob_consistency =
            BlobConsistencyConfig::construct(meta, &challenge_expressions, u8_table, range);
    ...
    fn synthesize(
        ...
        config.keccak_table.dev_load(
            &mut layouter,
            &self.blob_consistency.keccak_inputs(&blob_bytes),
            &challenge_values,
        )?;

        let barycentric_assignments = blob_consistency.assign_barycentric(); layouter.assign_region(
            || "barycentric config",
        ...
        layouter.assign_region(
            ...
            |mut region| {
                let assigned_rows = config.batch_data_config.assign_rows(
                    ...
                    challenge_values,
                    &self.data,
                    self.data.blob_consistency_witness.id,
                )?;
