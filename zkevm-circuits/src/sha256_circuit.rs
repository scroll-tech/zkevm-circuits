//! The SHA256 circuit is a wrapper for the circuit in sha256 crate and serve for precompile SHA-256 calls
//!
...
impl TableTrait for SHA256Table {
    fn cols(&self) -> [Column<Any>;4]{
        <Self as LookupTable<Fr>>::columns(self).as_slice().try_into().expect("return 4 cols")
    }
}
...
impl<F: Field> SHA256Circuit<F> {
    fn expected_rows(&self) -> usize {
        ...
        .reduce(|acc, v|acc+v)
        .unwrap_or_default() * TABLE16_BLOCK_ROWS
    }
}
