...
use bus_mapping::{mock::BlockData, circuit_input_builder::CircuitsParams};
...
fn block_2txs() -> Block<Fr> {
    ...
    let mut builder = BlockData::new_from_geth_data_with_params(block.clone(), params).new_circuit_input_builder();
    builder
        .handle_block(&block.eth_block, &block.geth_traces)
        .unwrap();
    block_convert(&builder.block, &builder.code_db).unwrap()
