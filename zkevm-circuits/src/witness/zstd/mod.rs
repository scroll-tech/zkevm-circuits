...
fn process_block_zstd_literals_header<F: Field>(
    ...
    let tag_rlc_iter = lh_bytes.iter().take(n_bytes_header).scan(Value::known(F::zero()), |acc, &byte| {
        *acc = *acc * randomness + Value::known(F::from(byte as u64));
        Some(*acc)
    });
...
fn process_block_zstd_huffman_code<F: Field>(
    ...
    huffman_header_row.state.tag_value_acc = tag_value_iter.next().expect("Next value should exist");
    ...
    let tag_value = tag_value_iter
        ...
        .last()
        .expect("Tag value must exist");
    let tag_rlc_iter = src.iter().skip(byte_offset + n_fse_bytes + 1).take(n_huffman_code_bytes).scan(
        Value::known(F::zero()),
        |acc, &byte| {
            *acc = *acc * randomness + Value::known(F::from(byte as u64));
            Some(*acc)
...
fn process_block_zstd_lstream<F: Field>(
    ...
    let tag_rlc_iter = src.iter().skip(byte_offset).take(len).scan(
        Value::known(F::zero()),
        |acc, &byte| {
            *acc = *acc * randomness + Value::known(F::from(byte as u64));
            Some(*acc)
    ...
    while current_bit_idx < len * N_BITS_PER_BYTE {
        if huffman_bitstring_map.contains_key(bitstring_acc.as_str()) {
            let sym = huffman_bitstring_map.get(bitstring_acc.as_str()).unwrap().clone();
...
mod tests {
    ...
    fn decode_literal_bitstream() -> Result<(), std::io::Error> {
        ...
        assert_eq!(ascii_symbols, expected_decoded_ascii, "Expect correct decoding");
    ...
    fn decode_literal_section() -> Result<(), std::io::Error> {
        let encoded: [u8; 555] = [
            ...
            0xae, 0x02, 0x0d, 0x11, 0x00, 0x76, 0x62, 0x5e, 0x23, 0x30, 0x6f,
        ...
        assert_eq!(decoded_literal_string, expected_literal_string, "Decode the correct literal string");
